...
|`group`  |array of `{value:(string,number),displayValue:(string,number)}`  |yes  |just a plain array,`value` and `displayValue` props are akin to `value` and `displayValue` props of **SelectMultipleButton**.  |
